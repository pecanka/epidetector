...
SUBROUTINE ScanData(n, s, x, y, nv, ierr, & 
  ...
  IMPLICIT NONE
  INTEGER, INTENT(IN)                      :: n
  INTEGER(iks), INTENT(INOUT)              :: x(n), y(n), s(n)
  ...
  INTEGER(iks)       :: subs1(n), x1co(n), y1co(n), x1ca(n), y1ca(n), x2co(n), &
                        y2co(n), x2ca(n), y2ca(n)
  REAL(dpp)          :: c01, min_cnt1, min_ca1, min_co1
  INTEGER            :: n1, n2, g1, g2, j, N_co, N_ca, N_co1, N_ca1, N_co2, &
                        N_ca2, n1co, n1ca, n2co, n2ca
  ...
  !CALL CleanUp(n, s, x, y, subs1, nv, n1, n2, ierr, s1, x1, y1, s2, x2, y2, &
  !             sex, excl_male, excl_fema)
  CALL CleanUp(n, s, x, y, subs1, nv, n1, n2, ierr, n1co, n1ca, x1co, y1co, &
               x1ca, y1ca, n2co, n2ca, x2co, y2co, x2ca, y2ca, sex, excl_male, &
               excl_fema)  
  ...
  !! Check for 'no valid data' and return if so
  IF(nv==0) THEN
    ierr = err_no_cc
  ...
  !! Assign the data into sample variables
  x1(1:n1co) = x1co(1:n1co)
  y1(1:n1co) = y1co(1:n1co)
  s1(1:n1co) = NumCo
  x1(n1co+1:n1co+n1ca) = x1ca(1:n1ca)
  y1(n1co+1:n1co+n1ca) = y1ca(1:n1ca)
  s1(n1co+1:n1co+n1ca) = NumCa
  x2(1:n2co) = x2co(1:n2co)
  y2(1:n2co) = y2co(1:n2co)
  s2(1:n2co) = NumCo
  x2(n2co+1:n2co+n2ca) = x2ca(1:n2ca)
  y2(n2co+1:n2co+n2ca) = y2ca(1:n2ca)
  s2(n2co+1:n2co+n2ca) = NumCa
  
  !! Get the counts
  cco1 = zero; cca1 = zero; cco2 = zero; cca2 = zero
  cco1(x1co(1:n1co)+1, y1co(1:n1co)+1) = cco1(x1co(1:n1co)+1, y1co(1:n1co)+1) + 1
  cca1(x1ca(1:n1ca)+1, y1ca(1:n1ca)+1) = cca1(x1ca(1:n1ca)+1, y1ca(1:n1ca)+1) + 1
  cco2(x2co(1:n2co)+1, y2co(1:n2co)+1) = cco2(x2co(1:n2co)+1, y2co(1:n2co)+1) + 1
  cca2(x2ca(1:n2ca)+1, y2ca(1:n2ca)+1) = cca2(x2ca(1:n2ca)+1, y2ca(1:n2ca)+1) + 1
...
!END SUBROUTINE ScanData
  ...
  SUBROUTINE CleanUp(n, s, x, y, ss, nv, n1, n2, ierr, n1co, n1ca, x1co, &
                     y1co, x1ca, y1ca, n2co, n2ca, x2co, y2co, x2ca, y2ca, &
                     sex, excl_male, excl_fema)
  ...
  ! ierr ... error code, ss ... subsample, sex ... sex indicator
    ...
    !INTEGER(iks), INTENT(INOUT), OPTIONAL :: s1(n), x1(n), y1(n), s2(n), x2(n), y2(n) 
    INTEGER(iks), INTENT(INOUT)           :: s(:), x(:), y(:), ss(:), & 
                                             x1co(:), y1co(:), x1ca(:), y1ca(:), &
                                             x2co(:), y2co(:), x2ca(:), y2ca(:) 
    INTEGER, INTENT(OUT)                  :: nv, n1, n2, ierr, n1co, n1ca, n2co, n2ca 
    ...
    IF(code==1) THEN
      ...
      N_co = 0; N_ca = 0; nv = 0; n1 = 0; n2 = 0
      n1co = 0; n1ca = 0; n2co = 0; n2ca = 0
      DO j=1,n
        ...
        IF(ss(j)==1_iks) THEN
          ...
          IF(s(j)==NumCo) THEN
            ...
            x1co(n1co) = x(j)
            y1co(n1co) = y(j)
          ELSEIF(s(j)==NumCa) THEN
            n1ca = n1ca + 1
            x1ca(n1ca) = x(j)
            y1ca(n1ca) = y(j)
          ENDIF
        
        !! Count S2 individuals  
        ELSE
    ...
    ELSEIF(code==2) THEN
      ...
      CALL PrntE("Add code to process x1, y1, s1, x2, y2, s2 in CleanUp.")
    ...
    ELSEIF(code==3) THEN
      ...
      !! Nulify the original variables (probably not even necessary)
      IF(nv<n) THEN
        x(nv+1:n)  = NumNA
        y(nv+1:n)  = NumNA
        s(nv+1:n)  = NumNA
