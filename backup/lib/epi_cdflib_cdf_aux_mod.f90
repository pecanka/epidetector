...
! .. Local Scalars ..
        ...
        TYPE (zf_locals) :: local1
! ..
        local1 = local
        print *,"========================"
        !print *,"local=", local1
        print *,"zf_status=", zf_status
        print *,"crash_left=", crash_left
        print *,"crash_hi=", crash_hi
...
90000       FORMAT (' Input argument out of range in routine: ', &
                    ...
                    ' is: ',ES15.8/' The upper bound on the value is: ', &
                    ES15.8/' The program is aborting')
...
90100       FORMAT (' Input argument out of range in routine: ', &
                    ...
                    ' is: ',ES15.8/' The lower bound on the value is: ', &
                    ES15.8/' The program is aborting')
            STOP 'Bound error on cdf routine -- see output'
          END IF
        END IF
...
! If the unknown is not cdf (which=1), check that cum and ccum add to one
        ...
        IF (which/=1) THEN
          tn1 = 'cum'
          tn2 = 'ccum'
          IF ( .NOT. add_to_one(params(1),params(2), &
               distrib%name,tn1,tn2,3,status)) RETURN
        END IF
