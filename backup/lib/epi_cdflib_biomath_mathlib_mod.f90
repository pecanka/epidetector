...
! FUNCTION log_gamma1(a) -- An exact renamed copy of gamln
...
! ..
        ...
        ELSE IF (ax<=four) THEN
          ...
          erf1 = half + (half-EXP(-x*x)*top/bot)
...
!                           -----------
!     GAMMA1(A) IS ASSIGNED THE VALUE 0 WHEN THE GAMMA FUNCTION CANNOT
!     BE COMPUTED.
!-----------------------------------------------------------------------
!     WRITTEN BY ALFRED H. MORRIS, JR.
...
!-----------------------------------------------------------------------
!            EVALUATION OF GAMMA1(A) FOR ABS(A) .LT. 15
!-----------------------------------------------------------------------
        t = one
        m = INT(a) - 1
...
!     COMPUTE GAMMA1(1 + X) FOR  0 .LE. X .LT. 1

30      CONTINUE

        gamma1 = evaluate_polynomial(p,x)/evaluate_polynomial(q,x)
...
!-----------------------------------------------------------------------
!            EVALUATION OF GAMMA1(A) FOR ABS(A) .GE. 15
!-----------------------------------------------------------------------
40      CONTINUE
...
!     FINAL ASSEMBLY
        ...
        IF (a<zero) gamma1 = (one/(gamma1*s))/x
...
!-----------------------------------------------------------------------
        ...
        ELSE
          ...
          log_gamma1 = (d+w) + (a-half)*(LOG(a)-one)

        END IF

      END FUNCTION log_gamma1
